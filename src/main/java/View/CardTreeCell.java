...
public class CardTreeCell extends TreeCell<String> {
    ...
    public static java.util.Set<CardsGroup> dropInsertIntoGroup(
        ...
        if (sourceElements != null && !sourceElements.isEmpty()) {
            ...
            // Track which element came from which source group so we can issue
            // OuicheList remove+add notifications for every cross-group move.
            java.util.Map<CardElement, CardsGroup> elementSourceGroup =
            ...
            for (CardElement ce : sourceElements) {
                for (java.util.Map.Entry<CardsGroup,
                    ...
                    if (entry.getValue().remove(ce)) {
                        ...
                        elementSourceGroup.put(ce, srcGroup);
                        break;
                    }
                }
            }
            ...
            // OuicheList notification: only for cross-group moves.
            // A same-group move is a pure reorder — the slot stays in the same
            // deck section / collection so the OuicheList is unaffected.
            java.util.Map<CardsGroup, java.util.List<CardElement>> removedPerGroup =
                    new java.util.LinkedHashMap<>();
            java.util.Map<CardsGroup, java.util.List<CardElement>> addedPerGroup =
                    new java.util.LinkedHashMap<>();
            for (CardElement movedElement : sourceElements) {
                CardsGroup srcGroup = elementSourceGroup.get(movedElement);
                if (srcGroup != null && srcGroup != targetGroup) {
                    removedPerGroup
                            .computeIfAbsent(srcGroup, k -> new java.util.ArrayList<>())
                            .add(movedElement);
                    addedPerGroup
                            .computeIfAbsent(targetGroup, k -> new java.util.ArrayList<>())
                            .add(movedElement);
                }
            }
            for (java.util.Map.Entry<CardsGroup, java.util.List<CardElement>> entry
                    : removedPerGroup.entrySet()) {
                CardGroupRegistry.notifyOuicheListOfGroupRemovals(
                        entry.getKey(), entry.getValue());
            }
            for (java.util.Map.Entry<CardsGroup, java.util.List<CardElement>> entry
                    : addedPerGroup.entrySet()) {
                CardGroupRegistry.notifyOuicheListOfGroupAdditions(
                        entry.getKey(), entry.getValue());
            }
